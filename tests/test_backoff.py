...
DEFAULT_BACKOFF_POLICY = BackoffPolicy(
    max_retries=3, base_delay=1, max_delay=10, factor=2, jitter=False
)

JITTER_BACKOFF_POLICY = BackoffPolicy(
    max_retries=3, base_delay=1, max_delay=10, factor=2, jitter=True
)
...
class TestExponentialBackoff(unittest.TestCase):
    ...
    def test_retries_on_failure(self, mock_sleep):
        ...
        self.assertEqual(mock_sleep.call_count, DEFAULT_BACKOFF_POLICY.max_retries - 1)
    ...
    def test_maximum_delay(self, mock_sleep):
        ...
        @exponential_backoff(
            backoff_policy=BackoffPolicy(
                max_retries=3, base_delay=1, max_delay=2, factor=2, jitter=False
            )
        )
        def failing_function():
            raise ValueError("Failure")
